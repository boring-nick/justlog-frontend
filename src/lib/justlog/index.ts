import type {
	JustlogAvailableLogsResponse,
	JustlogChannelsResponse,
	JustlogMessagesResponse,
	JustlogLogQuery,
	JustlogAvailableLogsQuery
} from './types';
export * from './types';
...
export class Justlog {
	...
	private async _getLogs(
		...
		query?: JustlogLogQuery,
		user?: string | number,
		year?: number,
		month?: number,
		random = false
	): Promise<JustlogMessagesResponse> {
		...
		if (random) {
			...
			if (month) path.push(month.toString());
		}

		return await this.fetch<JustlogMessagesResponse>(
			path.join('/') +
				'?json=1' +
	...
	async listLogs(query: JustlogAvailableLogsQuery): Promise<JustlogAvailableLogsResponse> {
		return await this.fetch<JustlogAvailableLogsResponse>(
			'list?' + new URLSearchParams(query as Record<string, string>).toString()
		);
	}
}
